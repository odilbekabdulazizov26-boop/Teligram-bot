...
async def download_song_by_name(query: str) -> tuple[Path | None, str]:
    """Search YouTube for a song by name and download as MP3. Returns (path, title)."""
    out_id = uuid.uuid4().hex
    out_template = str(DOWNLOAD_DIR / f"{out_id}.%(ext)s")
    ...
    matches = list(DOWNLOAD_DIR.glob(f"{out_id}.*"))
    if not matches:
        logger.error("yt-dlp finished but no audio file found")
        return None, ""

    return matches[0], title


async def recognize_song_from_audio(audio_path: Path) -> dict | None:
...
async def handle_song_search(update: Update, context: ContextTypes.DEFAULT_TYPE, query: str) -> None:
    """Search and send a song by name as MP3 audio."""
    status_msg = await update.message.reply_text(f"🔍 <b>{query}</b> qo'shig'i qidirilmoqda...", parse_mode="HTML")
    ...
    try:
        ...
        if song_path is None:
            await status_msg.edit_text(
                "❌ Qo'shiq topilmadi.\n\n"
                "Iltimos, boshqacha nom bilan yoki ijrochi nomini ham yozing.\n"
                "Masalan: <i>Shaxriyor Umarov — Muhabbat</i>",
                parse_mode="HTML",
            )
            return
...
async def handle_voice_message(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    """Handle voice messages: recognize song, then download and send it."""
    ...
    try:
        ...
        voice_filename = f"{uuid.uuid4().hex}.ogg"
        voice_path = DOWNLOAD_DIR / voice_filename
        await voice_file.download_to_drive(str(voice_path))

        await status_msg.edit_text("🔍 Qo'shiq tanib olinmoqda...")
        result = await recognize_song_from_audio(voice_path)

        if result is None:
            await status_msg.edit_text(
                "❌ Qo'shiqni tanib bo'lmadi.\n\n"
                "Shovqinli joyda yoki ovoz sifati past bo'lishi mumkin.\n"
        ...
        song_query = f"{artist} {title}" if artist else title
        ...
        song_path, _ = await download_song_by_name(song_query)

        if song_path is None:
            await status_msg.edit_text(
                f"✅ Qo'shiq aniqlandi: <b>{title}</b> — {artist}\n\n"
                "❌ Lekin yuklab bo'lmadi. Nomini qidiruv orqali sinab ko'ring.",
                parse_mode="HTML",
            )
            return
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await update.message.reply_html(
        ...
        "🎵 <b>Qo'shiq qidirish</b> — qo'shiq nomini yozing, topib beraman\n"
        "🎤 <b>Ovozdan qo'shiq aniqlash</b> — ovozli xabar yuboring, qo'shiqni tanib olaman\n"
...
async def help_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    await update.message.reply_html(
        ...
        "Qo'shiq nomini yozing — MP3 formatida topib beraman.\n"
        "Masalan: <i>Shaxriyor — Muhabbat</i>\n\n"
        "🎤 <b>Ovozdan qo'shiq aniqlash:</b>\n"
...
async def song_command(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    if context.args:
        ...
        await handle_song_search(update, context, query)
    else:
